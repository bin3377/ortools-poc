...
class VehicleScheduler:
    ...
    def _check_mobility_compatibility(self, vehicle_seat_types: Dict, mobility_assistance: List[str]) -> bool:
        """检查车辆座位类型是否支持乘客的移动辅助需求"""
        for assistance in mobility_assistance:
            assistance_lower = assistance.lower()
            if assistance_lower == "wheelchair" and not vehicle_seat_types.get("wheelchair", False):
                return False
            elif assistance_lower in ["cane or walker", "cane", "walker"] and not vehicle_seat_types.get("cane_walker", False):
                return False
            elif assistance_lower == "stretcher" and not vehicle_seat_types.get("stretcher", False):
                return False
            # 对于"ambulatory"（行走正常），所有车辆都支持
        return True

    def get_distance_and_duration(self, origin_lat: float, origin_lng: float,
    ...
    def schedule(self, vehicles: List[Dict], bookings: List[Dict],
                objective: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if objective is None:
            objective = {"minimize_vehicles": True, "minimize_cost": False, "minimize_time": False}
        ...
        for i in range(num_vehicles):
            for j in range(num_bookings):
                # 座位数量约束
                model.Add(vehicles_dict[i]['total_seats'] >= bookings_dict[j]['total_seat_count']).OnlyEnforceIf(x[i, j])
        ...
        for i in range(num_vehicles):
            for j1 in range(num_bookings):
                for j2 in range(num_bookings):
                    if j1 != j2:
                        # 简单的时间间隔约束，避免复杂的距离计算
                        pickup_time_j1 = self._time_to_minutes(bookings_dict[j1]['pickup_time'])
                        pickup_time_j2 = self._time_to_minutes(bookings_dict[j2]['pickup_time'])
                    ...
                    for j in range(num_bookings):
                        if solver.Value(x[i, j]) == 1:
                            vehicle_schedule['bookings'].append({
                                ...
                                'pickup_time': self._minutes_to_time(solver.Value(t[i, j])),
                                'pickup_address': bookings_dict[j]['pickup_address'],
                                'dropoff_address': bookings_dict[j]['dropoff_address'],
                                'passenger_count': bookings_dict[j]['total_seat_count']
                            })
                            vehicle_schedule['total_passengers'] += bookings_dict[j]['total_seat_count']
                            total_passengers += bookings_dict[j]['total_seat_count']

                    if vehicle_schedule['bookings']:
                        # 按时间排序预订
                        vehicle_schedule['bookings'].sort(key=lambda x: x['pickup_time'])
